...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { href: '/dashboard/qci-checklist', label: 'QCI Checklist', icon: <LayoutDashboard className="w-4 h-4" /> },
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Top Header */}
      <header className="hg-header px-6 py-2.5 flex items-center justify-between z-50">
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside className="hg-sidebar w-52 hidden md:flex flex-col shrink-0">
        ...
        {/* Mobile nav */}
        <div className="md:hidden flex overflow-x-auto" style={{ background: '#0F172A', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          {links.map((link) => {
        ...
        {/* Main Content */}
        <main className="flex-1 p-4 lg:p-6 overflow-y-auto" style={{ background: '#020617' }}>
          {children}
        </main>
      </div>

      {/* Footer */}
      <footer className="text-center py-3 text-xs" style={{ color: '#64748B', borderTop: '1px solid rgba(255,255,255,0.05)', background: '#020617' }}>
        Copyright © 2026 <span style={{ color: '#00F2FF' }}>NABH</span> All rights reserved.
      </footer>
    </div>
  );
}
